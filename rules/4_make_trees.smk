...
LINEAGES = []
OUTGROUPS = []
with open((os.getcwd()+"/"+(config["lineage_splits"]))) as lineage_fh:
...
#split_done defined but not used
rule fasttree:
    input:
...
#input metadata contains redundant fasta headers
...
rule summarize_make_trees:
    ...
    shell:
        ...
        echo "> Number of Philippines sequences in tree before expanding: $(gotree stats tips -i {input.grafted_tree} | tail -n+2 | grep -E "Philippines" | wc -l)\\n" >> {log}
        echo "> Total number of sequences in tree after expanding: $(gotree stats tips -i {input.expanded_tree} | tail -n+2 | wc -l)\\n" >> {log}
        echo "> Number of Philippines sequences in tree after expanding: $(gotree stats tips -i {input.expanded_tree} | tail -n+2 | grep -E "Philippines" | wc -l)\\n" >> {log}
        """
